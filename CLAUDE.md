...
**Noclip, freeze and speed work too, and they go through the state the client predicts**: dot-2d's `Dot2DAdminModifiers`, in each occupant's `Dot2DState.admin`, replicated by `RoomOccupantNet` as `net_admin`. Noclip is the furniture and the props not being there — `simulate_occupant` stops after the walls, inside the one function a client's replay runs — and not the walls, which are the room; noclip is for the person wedged in a bench. `headless_net` measures it tick by tick against a naive control (a crate only the client has, which is what a server-only noclip amounts to): 0.09 units at worst the shipped way, 102 the naive way; a freeze holds at 0.00, against a rubber band of 39–52 units for a server that alone refuses to move somebody — the lead's worth of walking the client predicts before each snapshot pulls it back. That control used to walk away without limit (530 units, and growing) because dot-net handed the predictor the client's own prediction as the server's answer for anything a snapshot left out; dot-net's CLAUDE.md #17. It now asserts both the rubber band and that the floor the client is pulled back to does not climb. `sandbox` repeats both over a real socket.
...
**And the room is now asked the other half of the question.** The wing, the gate, the gallery and the snug each asked whether a walker gets through a gap; nothing asked whether there is anywhere in the room a walker fits and *cannot reach*. That question produces a sealed pocket that every picture shows as floor. **anywhere a walker fits, a walker can reach** lays an 8-unit grid over the room, marks every point a walker's centre fits at, and floods from the hall: 21,707 points, all of them reached (21,027 since the alcove stood on 680 of them). With both snug gates closed it reports the pocket and where it starts.

#### The alcove: a bow off the south wall, and a gate at each end of it

The south-west quarter of the hall had a pillar in it and nothing else, and every place in the room to stand apart was a walk away from where people arrive. `ALCOVE_X` puts a bow of `ALCOVE_POST_RADIUS` posts on a circle of `ALCOVE_RADIUS` centred ON the south wall, west of the island. Its two ends stop `DOORWAY_SPAN` short of the wall — `alcove_end_angle()` derives the angle from the door's width, the way `SNUG_GATE_Y` derives the snug's gate — so somebody walking along the wall passes straight through it, in by one gate and out by the other, and touches nothing. The post count is derived too: as many as keep neighbours `GALLERY_OVERLAP` into each other, ten at these numbers.

**The neighbours placed it, and there is one place it fits.** The south-west pillar is the tightest: at x = -200 the nearest post stands 107 from it, at -210 exactly the door's 100, past that a slot. Moved east instead, the spawn ring's seat at (0, 392) ends up inside the east end, so one arrival in eight would start inside a wall. It sits 9 units clear of it now, which is also why the picture shows that arrival standing right at the bow's east end.

**Its radius is a lane and somewhere to stand out of it.** At the middle the bow's inner face is 170 off the wall: the door's width of lane along the wall, and 70 behind it — room for one person against the bow while somebody else walks through. It is left EMPTY, the gallery's rule, because the snug's seat rule (a person's width plus the door's between the seat and each wall) gives a seat of 13 here, which is a dot. At 218 the room for a seat existed and the pillar gap was 88.
